...
def get_recommendations(book_title, num_recommendations=6):
    ...
    recommended_books = [(df['title'][rec[0]],df['author'][rec[0]], round(rec[1] * 100)) for rec in recommendations] #Transforms the numbers of the recommendations into strings and adds them to recommendation_books list: book title + description + author + publish date
